...
for i=1:length(files2)
   CSI(1,i)=num2cell(load([folder2 files2(i).name]));
...
param.taw=linspace(0,2e-8,param.Max_time);
...
for t=1:1
    ...
    for p=1:40
     ...
     %dafe avaz mishe!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
        ...
        shapes=['O','*','>','x','+'];
        set(figure,'Color','w');
        title(strcat('The clustered data with 5 clusters for interval2'));
        for i=1:n_clusters
       ...
       [~,h_ind]=max(Likelihood); %index of highest likelihood value
         ...
         theta_i_interval2=mean_theta_origial(h_ind);  %direct path AoA
         Li_interval2=Likelihood(h_ind);
        
end 

Li_interval2
theta_i_interval2=theta_i_interval2*180/pi;
theta_i_interval2
res2=[Li_interval2,theta_i_interval2];
save('r2.mat','res2');
