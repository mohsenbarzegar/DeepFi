...
for t=1:1
    ...
    for p=1:40
       ...
       % [min_max max_max peaks_thresh];
        for tmpidx=1:length(r_i)
            if (abs(P_music(r_i(tmpidx),c_i(tmpidx)))>peaks_thresh)
                ...
                ToF_f(t,p,tmpidx)=param.taw(r_i(tmpidx))+1e-14; %3D
                data_f=[data_f
     ...
     %dafe avaz mishe!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
        ...
        [center,U,obj_fcn] = fcm(data_f, n_clusters);
        ...
        shapes=['O','*','>','x','+'];
        set(figure,'Color','w');
        title(strcat('The clustered data with 5 clusters for interval1'));
        for i=1:n_clusters
       ...
       [~,h_ind]=max(Likelihood); %index of highest likelihood value
         ...
         theta_i_interval1=mean_theta_origial(h_ind);  %direct path AoA
         Li_interval1=Likelihood(h_ind);
...
theta_i_interval1
res1=[Li_interval1,theta_i_interval1];
save('r1.mat','res1');
