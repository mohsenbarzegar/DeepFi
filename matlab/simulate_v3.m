...
param.taw=linspace(0,7e-8,param.Max_time);
    ...
    %surf(abs(P_music))
     ...
     %dafe avaz mishe!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
        ...
        for i=1:1:n_clusters
            ...
            C(i)=length(index{i});
            wc=1; w_theta=1/max(sigma_theta);
       ...
       [~,h_ind]=max(Likelihood); %index of highest likelihood value
         ...
         theta_i(t)=mean_theta_origial(h_ind);  %direct path AoA
...
Li
theta_i*180/pi
%Thetha_avg=(sum(Li.*theta_i)/sum(Li))*180/pi;
[t_i k_i]=max(Li);
theta_left=theta_i(find(theta_i>0));
theta_right=theta_i(find(theta_i<0));
l1=length(theta_left);
l2=length(theta_right);
% if(l1>l2)
%     Thetha_avg=(mean(theta_left))*180/pi
%     disp('left hand of AP');
% end
% 
% if(l1<l2)
%     Thetha_avg=(mean(theta_right))*180/pi
%     disp('right hand of AP');
% end

%disp('asnwer is:')
%disp(num2str(theta_i*180/pi));
%disp(avg_angle:num2str(Thetha_avg*180/pi));
%disp(max_prob_angle:num2str(theta_i(k_i)*180/pi));
